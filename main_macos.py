...
def documentation_window():
    ...
    about.geometry("340x240")

    about_frame = tk.Frame(about)
    about_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

    twitter_link = tk.Label(about_frame, text="PyAutoGUI2", cursor="hand2", fg="blue")
    twitter_link.pack(pady=5)
    twitter_link.bind("<Button-1>", lambda e: webbrowser.open("https://pyautogui2.com"))

    github_link = tk.Label(about_frame, text="PyAutoGUI", cursor="hand2", fg="blue")
    ...
    about.geometry("+%d+%d" % (root.winfo_rootx() + 220, root.winfo_rooty() + 100))
...
def developer_window():
    ...
    label = tk.Label(about_frame, text="Created by Endre Bodi,\nbased on PyAutoGUI by Al Sweigart.")
    label.pack(pady=10)

    github_link = tk.Label(about_frame, text="GitHub", cursor="hand2", fg="blue")
    github_link.pack(pady=5)
    github_link.bind("<Button-1>", lambda e: webbrowser.open("https://github.com/endrebodi"))

    twitter_link = tk.Label(about_frame, text="Twitter", cursor="hand2", fg="blue")
    twitter_link.pack(pady=5)
...
def add_option():
    ...
    if selected_option == "Move cursor to coordinate":
        ...
        textbox.insert(tk.END, "pyautogui.hotkey('command', 'c')\n")
    elif selected_option == "Paste":
        textbox.insert(tk.END, "pyautogui.hotkey('command', 'v')\n")
    else:
...
def drag_mouse():
    ...
    cursor_position_x = cursor_position[0]
    cursor_position_y = cursor_position[1]
    textbox.insert(tk.END, f"pyautogui.dragTo({cursor_position_x},{cursor_position_y}, 0.2, button = 'left')\n")
